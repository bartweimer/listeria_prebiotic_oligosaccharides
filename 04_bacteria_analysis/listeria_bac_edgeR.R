...
setwd("/Users/taylorreiter/Desktop/Davis/Rotations/Weimer/Po_seq/Listeria/bac_listeria/featureCounts_output/")
...
counts_bac<-read.table("fc_listeriaCOUNTS.txt")
annotations<-counts_bac[,1]
 
# Change column names from file name to sample name
previous.names<-colnames(counts_bac)
samples_names<-c("S7","S6","S5","S23","S24","S25","S26","S27","S20","S21","S22","S28","S29","S30","S31", "S32", "S33", "S8", "S34")
counts_bac<-as.data.frame(counts_bac)
library(data.table)
setnames(counts_bac, previous.names, samples_names)

# Export as csv so renaming won't need to be repeated
write.csv(counts_bac, file="fc_run_all_listeriaCOUNTSrenamed.csv")

# Read in csv with annotation information
counts_bac<-read.csv("fc_run_all_listeriaCOUNTSrenamed.csv")
head(counts_bac)

# Subset for each treatment & create a dataframe; create group object for treatment
# Treatment 6
treatment6<-c("S7", "S6", "S5", "S8", "S28", "S29", "S30", "S31")
treatment6_counts<-counts_bac[, (names(counts_bac) %in% treatment6)]
...
treatment7_counts<-counts_bac[, (names(counts_bac) %in% treatment7)]
...
treatment8_counts<-counts_bac[, (names(counts_bac) %in% treatment8)]
...
treatment9_counts<-counts_bac[, (names(counts_bac) %in% treatment9)]
...
treatment10_counts<-counts_bac[, (names(counts_bac) %in% treatment10)]
...
treatment11_counts<-counts_bac[, (names(counts_bac) %in% treatment11)]
...
# number annotations to facilitate merging
library(dplyr)
counts_bac <- add_rownames(counts_bac, "number")


et6 <- exactTest(treatment6_counts, pair=c("control", "listeria"))
tags6<-topTags(et6, sort.by="PValue", p.value=1, n=10000)
tags6_annot<-add_rownames(tags6[[1]], "number")
tags6_annot<-merge(tags6_annot, counts_bac[,1:2], by="number")
write.csv(tags6_annot, file="t6_bac_listeria_fdr1.csv", row.names=F)


et7 <- exactTest(treatment7_counts, pair=c("control", "listeria"))
tags7<-topTags(et7, sort.by="PValue", p.value=1, n=10000)
tags7_annot<-add_rownames(tags7[[1]], "number")
tags7_annot<-merge(tags7_annot, counts_bac[,1:2], by="number")
write.csv(tags7_annot, file="t7_bac_listeria_fdr1.csv", row.names=F)

et8 <- exactTest(treatment8_counts, pair=c("control", "listeria"))
tags8<-topTags(et8, sort.by="PValue", p.value=1, n=10000)
tags8_annot<-add_rownames(tags8[[1]], "number")
tags8_annot<-merge(tags8_annot, counts_bac[,1:2], by="number")
write.csv(tags8_annot, file="t8_bac_listeria_fdr1.csv", row.names=F)

head(tags8[[1]])
et9 <- exactTest(treatment9_counts, pair=c("control", "listeria"))
tags9<-topTags(et9, sort.by="PValue", p.value=1, n=10000)
tags9_annot<-add_rownames(tags9[[1]], "number")
tags9_annot<-merge(tags9_annot, counts_bac[,1:2], by="number")
write.csv(tags9_annot, file="t9_bac_listeria_fdr1.csv", row.names=F)

et10 <- exactTest(treatment10_counts, pair=c("control", "listeria"))
tags10<-topTags(et10, sort.by="PValue", p.value=1, n=10000)
tags10_annot<-add_rownames(tags10[[1]], "number")
tags10_annot<-merge(tags10_annot, counts_bac[,1:2], by="number")
write.csv(tags10_annot, file="t10_bac_listeria_fdr1.csv", row.names=F)

et11 <- exactTest(treatment11_counts, pair=c("control", "listeria"))
tags11<-topTags(et11, sort.by="PValue", p.value=1, n=10000)
tags11_annot<-add_rownames(tags11[[1]], "number")
tags11_annot<-merge(tags11_annot, counts_bac[,1:2], by="number")
write.csv(tags11_annot, file="t11_bac_listeria_fdr1.csv", row.names=F)
